...
contract RealEstate {
    ...
    function listProperty(uint256 price, string memory _propertyTitle, string memory _category, string memory _images, string memory _propertyAddress, string memory _description) external returns (uint256){
        ...
        emit PropertyListed(productId, msg.sender, price);

        return productId;
    }

    function updateProperty(uint256 productId, string memory _propertyTitle, string memory _category, string memory _images, string memory _propertyAddress, string memory _description) external returns (uint256){
        Property storage property = properties[productId];

        require(property.owner == msg.sender, "You are not the owner");
    ...
    function updatePrice(uint256 productId, uint256 price) external returns(string memory){
        Property storage property = properties[productId];
    ...
    function buyProperty(uint256 productId) external payable{
        ...
        uint256 price = properties[productId].price;
        ...
        if(sent){
            property.owner = msg.sender;
            emit PropertySold(productId, oldOwner, msg.sender, price);

            if(amount > price){
                (bool refundSent,) = payable(msg.sender).call{value: amount - price}("");
